...
ofxWater& ofxWater::allocate(int tWidth, int tHeight){ 
    ...
    for(int i = 0; i < 2; i++){
        mapFbo[i].allocate(width,height,GL_RGB);
        mapFbo[i].begin();
        ofClear(0);
        mapFbo[i].end();
    }
    ...
    //updateShader.load("","shaders/waterUpdate.frag");
    ...
    //renderShader.load("","shaders/waterRender.frag");
    ...
    \
    void main(){ \
        vec3 color = texture2DRect(tex, gl_TexCoord[0].st ).rgb;\
        if (color.r > 0.0)\
            gl_FragColor.a = 1.0;\
        else\
            gl_FragColor.a = 0.0;\
    ...
    //maskShader.load("","shaders/mask.frag");
    ...
    //blurShader.load("","shaders/fastBlur.frag");
...
void ofxWater::begin(){
    ...
    maskShader.begin();
    maskShader.setUniform1f("threshold", threshold);
